...
const ContactForm: React.FC<ContactFormProps> = ({ onSuccess, onError }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Något gick fel vid sändning.');
      }
    ...
    <div className="bg-white tech-border p-8 md:p-12 tech-shadow">
      {status === 'success' ? (
        ...
        >
          ...
          <h3 className="text-3xl font-display font-bold uppercase mb-4">Tack för ditt meddelande!</h3>
          <p className="font-mono text-ink/70">Vi har tagit emot ditt mail och återkommer så snart vi kan.</p>
          <button 
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label htmlFor="name" className="font-mono text-xs font-bold uppercase tracking-widest text-ink/70">Namn</label>
              <input 
                type="text" 
                id="name"
                name="name"
            ...
            <div className="space-y-2">
              <label htmlFor="email" className="font-mono text-xs font-bold uppercase tracking-widest text-ink/70">E-post</label>
              <input 
                type="email" 
                id="email"
                name="email"
          ...
          <div className="space-y-2">
            <label htmlFor="subject" className="font-mono text-xs font-bold uppercase tracking-widest text-ink/70">Ämne</label>
            <input 
              type="text" 
              id="subject"
              name="subject"
          ...
          <div className="space-y-2">
            <label htmlFor="message" className="font-mono text-xs font-bold uppercase tracking-widest text-ink/70">Meddelande</label>
            <textarea 
              id="message"
              name="message"
          ...
          >
            {status === 'submitting' ? (
              <><RefreshCw className="w-6 h-6 animate-spin" /> Skickar...</>
            ) : (
              <><>Skicka Meddelande <ArrowRight className="w-6 h-6" /></></>
            )}
          </button>
        </form>
      )}
