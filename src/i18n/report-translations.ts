...
export function localizeScanResult(result: ScanResult, requestedLanguage: string): LocalizedScanResult {
  // Deep-scan architecture already localizes titles/descriptions/agentFix on the server.
  const needsClientLocalization = result.issues.some(
    (issue) => issue.id && (!issue.title || issue.recommendationAvailable)
  );
  if (!needsClientLocalization) {
    return result;
  }
  ...
  const issues: LocalizedScannerIssue[] = result.issues.map((issue) => {
    ...
    return {
      ...
      recommendation: issue.recommendationAvailable ? text.recommendation : issue.recommendation,
  ...
  return {
    ...
    summary: result.summary || translation.summaries[getSummaryIndex(result.overallScore)],
    issues
  };
}
